...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name zExchange return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name zExchange return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name zExchange';
  }
`
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.pancakeswap_RETURN) {
          return p0?.zExchange?.return > p1?.zExchange?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
